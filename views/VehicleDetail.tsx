...
const VehicleDetail: React.FC<Props> = ({ 
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 w-full max-w-md mx-auto relative shadow-2xl overflow-hidden">
      ...
      {/* 
        FIXED HEADER: 
        Consistent h-16 (64px) + Safe Area Top
        Identical to Layout.tsx for smooth transitions
      */}
        ...
        <div className="pt-[env(safe-area-inset-top)] w-full">
            <div className="h-16 px-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    {activeSectionId ? (
                        // Back to Dashboard
                        <button onClick={() => setActiveSectionId(null)} className="shrink-0 p-2 -ml-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors">
                            <ArrowLeft size={24} />
                        </button>
                    ) : (
                        // Back to Garage
                        <button onClick={onBack} className="shrink-0 p-2 -ml-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors">
                            <ArrowLeft size={24} />
                        </button>
                    )}
                    ...
                    {activeSectionId ? (
                        // Section Title
                        <div className="flex items-center gap-3 min-w-0 animate-in fade-in slide-in-from-left-4 duration-300">
                        ...
                        <div className="flex flex-col min-w-0 animate-in fade-in slide-in-from-left-4 duration-300">
                            <div className="flex items-center gap-2">
                                <h2 className="text-lg font-bold text-slate-900 dark:text-white leading-none truncate pr-1 mt-0.5">
                            ...
                            <p className="text-xs font-semibold text-slate-400 truncate leading-none mt-1">{vehicle.plate}</p>
                        </div>
                    )}
                </div>
                ...
                {/* Right Side: Actions */}
                {!activeSectionId && (
                     <div className="flex gap-2 shrink-0 animate-in fade-in slide-in-from-right-4 duration-300">
                        <button 
                            onClick={() => setShowModsModal(true)}
                            className="px-4 py-2 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-500/30 transition-colors font-bold text-xs"
                        >
                            {t('mods_short')}
                        </button>
                        <button 
      ...
      <main 
          className="flex-1 w-full overflow-y-auto overflow-x-hidden px-6 pb-6 space-y-6 no-scrollbar"
          style={{
